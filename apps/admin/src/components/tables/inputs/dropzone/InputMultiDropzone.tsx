...
export default function InputMultiDropzone(props: InputProps) {
  ...
  const handleDeleteAll = () => {
    setValues(null);
    form.setValue(props.formValueName, null, {
      shouldDirty: true,
      shouldValidate: true,
      shouldTouch: true,
    });
  ...
  const handleDeleteSingle = (i: number) => {
    const filteredValues = values
      ...
      : null;
    const newValues =
      filteredValues && filteredValues.length > 0 ? filteredValues : null;
    setValues(newValues);
    form.setValue(props.formValueName, newValues, {
      shouldDirty: true,
      shouldValidate: true,
      shouldTouch: true,
    });
  ...
  const handleReset = () => {
    ...
    form.setValue(props.formValueName, props.defaultValues, {
      shouldDirty: true,
      shouldValidate: true,
      shouldTouch: true,
    });
  ...
  const sortedDefaultValues = props.defaultValues?.sort((a, b) =>
            ...
            >
              ...
              >
                {values.map((value, index) => {
                  if (!props.files && value.url.length > 0)
                    return (
                      <figure
                        key={index}

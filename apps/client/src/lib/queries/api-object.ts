...
//.........................DENDTOCHRONOLOGIES.........................//
export const getDendrochronologyById = async (id: string): Promise<DendrochronologyById> => {
  const headers = { "Content-Type": "application/json" };
  const query = /* GraphGL */ `
    query DendrochronologyById {
      dendrochronologies(where: { id: "${id}", status: listed }) {
            totalCount
            edges {
              node {
                id
  ...
  if (!res.ok) {
    ...
    console.log(err);
    // Throw an error
    throw new Error(`Failed to fetch data 'Dendrochronology ${id}'`);
  }
  ...
  if (
    json.data.dendrochronologies.totalCount === 0 ||
    json.data.dendrochronologies.edges.length === 0
  ) {
    notFound();
  }

  const dendrochronology = DendrochronologyById.parse(json.data?.dendrochronologies.edges[0].node);

  return dendrochronology;
};
...
export function isParamInString(
...
export function isParamInArray(
  param: string | undefined,
  array: { id: string }[],
) {
  if (!!param) {
    const paramArray = param.split("_");
    const isInParam = paramArray.some((id) => array.some((el) => el.id === id));
    return isInParam;
  } else return true;
}
...
export function filterArtifacts(
  ...
  const filtered = result.edges.map((elem) => {
    if (ArtiNode.safeParse(elem.node).success) {
      ...
      // GLOBAL
      const byName = artifactsNode.artifacts.filter((artifact) =>
        artifact.displayName.toLowerCase().includes(search ?? ""),
      );
      const byCategories_Collections = byName.filter(
        (artifact) =>
          isParamInString(categories, artifact.collection.category.slug) &&
          isParamInString(collections, artifact.collection.slug),
      );

      //LOCATIONS
      const byCountry = byCategories_Collections.filter((artifact) =>
        isParamInString(countryIds, artifact.location?.country?.id),
      );
      const byRegion = byCountry.filter((artifact) =>
        isParamInString(regionIds, artifact.location?.region?.id),
      );
      const byDistrict = byRegion.filter((artifact) =>
        isParamInString(districtIds, artifact.location?.district?.id),
      );
      const bySettlement = byDistrict.filter((artifact) =>
        isParamInString(settlementIds, artifact.location?.settlement?.id),
      );

      //LICENSE
      const byLicense =
        artifactsNode.__typename === "License"
          ? bySettlement
          : bySettlement.filter((artifact) =>
              isParamInString(licenseIds, artifact.license?.id),
            );

      // ARTIFACTS FIELDS
      const byCultures =
        artifactsNode.__typename === "Culture"
          ? byLicense
          : byLicense.filter((artifact) =>
              isParamInString(cultureIds, artifact.culturalAffiliation?.id),
            );
      const bySets =
        artifactsNode.__typename === "Set"
          ? byCultures
          : byCultures.filter((artifact) =>
              isParamInString(setIds, artifact.set?.id),
            );
      const byMonuments =
        artifactsNode.__typename === "Monument"
          ? bySets
          : bySets.filter((artifact) =>
              isParamInString(monumentIds, artifact.monument?.id),
            );
      const byTechniques =
        artifactsNode.__typename === "Technique"
          ? byMonuments
          : byMonuments.filter((artifact) =>
              isParamInArray(techniqueIds, artifact.techniques),
            );
      ...
      const byName = locationsArtiNode.locations.map((location) => {
        return location.artifacts.filter((artifact) =>
          artifact.displayName.toLowerCase().includes(search ?? ""),
        );
      });
      ...
      //LOCATIONS
      const byCountry =
        locationsArtiNode.__typename === "Country"
      ...
      const byRegion =
        locationsArtiNode.__typename === "Region"
          ? byCountry
      ...
      const byDistrict =
        locationsArtiNode.__typename === "District"
          ? byRegion
      ...
      const bySettlement =
        locationsArtiNode.__typename === "Settlement"
          ? byDistrict
          : byDistrict.map((artifacts) => {
              return artifacts.filter((artifact) =>
                isParamInString(
                  settlementIds,
                  artifact.location?.settlement?.id,
                ),
              );
            });
...
export function filterBooks(
  ...
  const filtered = result.edges.map((elem) => {
    if (BooksNode.safeParse(elem.node).success) {
      ...
      // GLOBAL
      const byName = booksNode.books.filter((book) =>
        book.displayName.toLowerCase().includes(search ?? ""),
      );
      const byCategories_Collections = byName.filter(
        (book) =>
          isParamInString(categories, book.collection.category.slug) &&
          isParamInString(collections, book.collection.slug),
      );

      //LOCATIONS
      const byCountry = byCategories_Collections.filter((book) =>
        isParamInString(countryIds, book.location?.country?.id),
      );
      const byRegion = byCountry.filter((book) =>
        isParamInString(regionIds, book.location?.region?.id),
      );
      const byDistrict = byRegion.filter((book) =>
        isParamInString(districtIds, book.location?.district?.id),
      );
      const bySettlement = byDistrict.filter((book) =>
        isParamInString(settlementIds, book.location?.settlement?.id),
      );

      //LICENSE
      const byLicense =
        booksNode.__typename === "License"
          ? bySettlement
          : bySettlement.filter((book) =>
              isParamInString(licenseIds, book.license?.id),
            );

      // BOOKS FIELDS
      const byBookGenre =
        booksNode.__typename === "BookGenre"
          ? byLicense
          : byLicense.filter((book) =>
              isParamInArray(bookGenreIds, book.bookGenres),
            );
      ...
      const byName = locationsBooksNode.locations.map((location) => {
        return location.books.filter((book) =>
          book.displayName.toLowerCase().includes(search ?? ""),
        );
      });
      ...
      //LOCATIONS
      const byCountry =
        locationsBooksNode.__typename === "Country"
          ? byCategories_Collections
      ...
      const byRegion =
        locationsBooksNode.__typename === "Region"
          ? byCountry
          : byCountry.map((books) => {
              return books.filter((book) =>
                isParamInString(regionIds, book.location?.region?.id),
      ...
      const byDistrict =
        locationsBooksNode.__typename === "District"
          ? byRegion
          : byRegion.map((books) => {
              return books.filter((book) =>
                isParamInString(districtIds, book.location?.district?.id),
      ...
      const bySettlement =
        locationsBooksNode.__typename === "Settlement"
          ? byDistrict
...
export function filterProtectedAreaPictures(
  result: PAPFilters,
  searchParams: { [key: string]: string | string[] | undefined },
) {
  const search = searchParams["search"] as string | undefined;
  ...
  const filtered = result.edges.map((elem) => {
    if (PAPNode.safeParse(elem.node).success) {
      ...
      // GLOBAL
      const byName = protectedAreaPictureNode.protectedAreaPictures.filter(
        (pap) => pap.displayName.toLowerCase().includes(search ?? ""),
      );
      const byCategories_Collections = byName.filter(
        (pap) =>
          isParamInString(categories, pap.collection.category.slug) &&
          isParamInString(collections, pap.collection.slug),
      );

      //LOCATIONS
      const byCountry = byCategories_Collections.filter((pap) =>
        isParamInString(countryIds, pap.location?.country?.id),
      );
      const byRegion = byCountry.filter((pap) =>
        isParamInString(regionIds, pap.location?.region?.id),
      );
      const byDistrict = byRegion.filter((pap) =>
        isParamInString(districtIds, pap.location?.district?.id),
      );
      const bySettlement = byDistrict.filter((pap) =>
        isParamInString(settlementIds, pap.location?.settlement?.id),
      );

      //LICENSE
      const byLicense =
        protectedAreaPictureNode.__typename === "License"
          ? bySettlement
          : bySettlement.filter((pap) =>
              isParamInString(licenseIds, pap.license?.id),
            );

      // PAP FIELDS
      const byProtectedArea =
        protectedAreaPictureNode.__typename === "ProtectedArea"
          ? byLicense
          : byLicense.filter((pap) =>
              isParamInString(protectedAreaIds, pap.protectedArea?.id),
            );
      const byProtectedAreaCategory = byProtectedArea.filter((pap) => {
        if (
          !!protectedAreaCategoryIds &&
          pap.protectedArea?.protectedAreaCategory === null
        ) {
          return false;
        } else {
      ...
      const byName = categorysPAPNode.protectedAreas.map((paps) => {
        return paps.protectedAreaPictures.filter((pap) =>
          pap.displayName.toLowerCase().includes(search ?? ""),
        );
      });
      ...
      const byName = locationsPAPNode.locations.map((location) => {
        return location.protectedAreaPictures.filter((pap) =>
          pap.displayName.toLowerCase().includes(search ?? ""),
        );
      });
      const byCategories_Collections = byName.map((paps) => {
      ...
      //LOCATIONS
      const byCountry =
        locationsPAPNode.__typename === "Country"
          ? byCategories_Collections
      ...
      const byRegion =
        locationsPAPNode.__typename === "Region"
          ? byCountry
          : byCountry.map((paps) => {
              return paps.filter((pap) =>
                isParamInString(regionIds, pap.location?.region?.id),
      ...
      const byDistrict =
        locationsPAPNode.__typename === "District"
          ? byRegion
          : byRegion.map((paps) => {
              return paps.filter((pap) =>
                isParamInString(districtIds, pap.location?.district?.id),
      ...
      const bySettlement =
        locationsPAPNode.__typename === "Settlement"
          ? byDistrict
          : byDistrict.map((paps) => {
              return paps.filter((pap) =>
                isParamInString(settlementIds, pap.location?.settlement?.id),
